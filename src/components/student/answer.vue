            ...
            <div class="item">
              ...
              <ul>
                <li v-for="(list, index2) in topic[2]" :key="index2">
                  <a href="javascript:" @click="judge(index2)"
                     :class="{'border': index === index2 && currentType === 2,'bg': bg_flag && topic[2][index2].isClick === true}"><span
            ...
            <div class="item">
              ...
              <ul>
                <li v-for="(list, index3) in topic[3]" :key="index3">
                  <a href="javascript:" @click="multiple(index3)"
                     :class="{'border': index === index3 && currentType === 3,'bg': multipleAnswer[index3] === true}"><span
            ...
            <div class="item">
              ...
              <ul>
                <li v-for="(list, index4) in topic[4]" :key="index4">
                  <a href="javascript:" @click="fill(index4)"
                     :class="{'border': index === index4 && currentType === 4,'bg': fillAnswer[index4] === true}"><span
                    :class="{'mark': topic[4][index4].isMark === true}"></span>{{topicCount[0]+topicCount[1]+topicCount[2]+index4+1}}</a>
                </li>
              </ul>
            </div>
        ...
        <div class="right">
          ...
          <div class="content">
            ...
            <div class="judge" v-if="currentType === 2">
              <el-radio-group v-model="judgeAnswer[index]" @change="getJudgeLabel" v-if="currentType === 2">
                <el-radio :label="1">正确</el-radio>
                <el-radio :label="2">错误</el-radio>
              </el-radio-group>
            </div>
            <div v-if="currentType === 3">
              <el-radio-group v-model="multipleAnswer[index]" @change="">
                <el-radio :label="1">{{showAnswer.answerA}}</el-radio>
                <el-radio :label="2">{{showAnswer.answerB}}</el-radio>
                <el-radio :label="3">{{showAnswer.answerC}}</el-radio>
                <el-radio :label="4">{{showAnswer.answerD}}</el-radio>
            ...
            <div class="fill" v-if="currentType === 4">
      ...
      mark() { //标记功能
        switch (this.currentType) {
          ...
          case 2:
            ...
            break;
          case 3:
            this.topic[3][this.index]["isMark"] = true; //多选题标记
            break;
          case 4:
            this.topic[4][this.index]["isMark"] = true;//填空题标记
        }
      },
      showTime() { //倒计时
        setInterval(() => {
      ...
      getExamData() { //获取当前试卷所有信息
        ...
        this.$axios(`/api/findExamById/${examCode}`).then(res => {  //通过examCode请求试卷详细信息
          ...
          this.$axios(`/api/queryExamPaper/${examCode}`).then(res => {  //通过paperId获取试题题目信息
            ...
            console.log('试题内容', this.reduceAnswer);
            ...
            console.log('这是啥', this.topicCount);
            let len = this.topicCount[3];
            let father = [];
            for (let i = 0; i < len; i++) { //根据填空题数量创建空数组存放每道题答案
              let children = [null];
              father.push(children)
            }
            this.fillAnswer = father;

            let mul = this.topicCount[4];
            let multipleFa = [];
            for (let i = 0; i < mul; i++) { //根据多选题数量创建空数组存放每道题答案
              let children = [null];
              multipleFa.push(children)
            }
            this.multipleAnswer = multipleFa;
      ...
      fill(index) { //填空题
        ...
        if (index < len) {
          if (index < 0) {
            ...
            this.showQuestion = Data[index].question; //获取题目信息
            //根据题目中括号的数量确定填空横线数量
            this.part = this.showQuestion.split("____").length - 1;
